...
import { CartContext, useCart } from "@app/hooks/useCarts"
...
export const CartIcon = () => {
  ...
  const cartDropDownItem = useRef(null)
  ...
  return (
    <IconContainer ref={cartDropDownItem} onMouseOver={handleCartDropDown}>
      ...
      </div>
      {
        cartProducts?.map((item) => {
          return (
            showCartDropdown && <CartDropdown key={item.id}>
              <li className="flex justify-between">
                ...
                <p className="text-[tomato] mt-1">x {item.product_count}</p>
              </li>
            </CartDropdown>
          )
        })
